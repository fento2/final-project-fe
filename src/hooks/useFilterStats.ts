    ...
    useEffect(() => {
        if (!loading && jobs.length > 0) {
            ...
            jobs.forEach(job => {
                // Count categories
                if (job.category) {
                    newStats.categories[job.category] = (newStats.categories[job.category] || 0) + 1;
                }

                // Count job types
                if (job.job_type) {
                    newStats.jobTypes[job.job_type] = (newStats.jobTypes[job.job_type] || 0) + 1;
                }

                // Count locations
                if (job.location) {
                    newStats.locations[job.location] = (newStats.locations[job.location] || 0) + 1;
                }
