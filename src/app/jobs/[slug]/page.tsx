...
export default function JobDetailPage() {
    ...
    useEffect(() => {
        if (currentJob?.skills && Array.isArray(currentJob.skills) && currentJob.skills.length > 0) {
            // Get skill names for filtering
            const skillNames = currentJob.skills.map(s => s.name);
            // For now, we'll filter by category as skills-based filtering would need backend support
            related.refetch({ category: currentJob.category, limit: 8 });
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [currentJob?.skills, currentJob?.category]);

    const jobTitle = useMemo(() => currentJob?.title || decodeURIComponent(slug).replace(/-/g, " "), [currentJob?.title, slug]);
    ...
    const companyName = currentJob.Company?.name || "Unknown Company";
    const companyLogo = toAbsoluteUrl(currentJob.Company?.profile_picture);
    const location = currentJob.location || "-";
    const currency = currentJob.currency || "IDR";
    const salaryStr = formatCurrency(currentJob.salary, { currency });
    const period = currentJob.periodSalary ? currentJob.periodSalary.toLowerCase().replace(/_/g, " ") : "month";
    const postedAt = currentJob.createdAt ? formatDateIDDateOnly(currentJob.createdAt) : "";
    const lastActivity = currentJob.updatedAt ? formatDateIDDateOnly(currentJob.updatedAt) : postedAt;
    const closesAt = currentJob.expiredAt ? formatDateIDDateOnly(currentJob.expiredAt) : "-";
                ...
                <div className="max-w-6xl mx-auto px-6 py-16">
                    ...
                    <nav className="text-sm text-white/80 mb-6">
                        <ol className="flex space-x-2">
                            ...
                            <li>/</li>
                            <li className="text-white font-semibold">{currentJob.category}</li>
                        </ol>
                    </nav>
                    ...
                    <div className="text-white">
                        ...
                        <div className="flex flex-wrap gap-2 mb-8">
                            {currentJob.category && (
                                <span className="px-3 py-1 text-sm font-medium bg-blue-600 text-white rounded-md">{currentJob.category}</span>
                            )}
                            {currentJob.job_type && (
                                <span className="px-3 py-1 text-sm font-medium bg-purple-600 text-white rounded-md">{currentJob.job_type}</span>
                            )}
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    <div className="flex-1">
                        ...
                        <div className="bg-white rounded-xl shadow-sm p-8 mb-8">
                            ...
                            {currentJob.description ? (
                                <SafeHtml 
                                    ...
                                    className="text-gray-700 leading-relaxed prose prose-lg max-w-none [&_*]:text-inherit"
                                />
                            ) : (
                        ...
                        <div className="bg-white rounded-xl shadow-sm p-8 mb-8">
                            ...
                            {Array.isArray(currentJob.skills) && currentJob.skills.length > 0 ? (
                        ...
                        <div className="bg-white rounded-xl shadow-sm p-8">
                            ...
                            {currentJob.Company?.description ? (
                                <SafeHtml 
                                    html={currentJob.Company.description}
