...
export const useSavedJobs = () => {
    ...
    const normalize = (raw: any): SavedJob[] => {
        ...
        return arr.map((item: any) => ({
            ...
            Job: {
                ...
                Company: {
                    name: item.Jobs?.Companies?.name || item.Companies?.name || item.companyName || 'Unknown Company',
                    profile_picture: item.Jobs?.Companies?.profile_picture || item.Companies?.profile_picture || item.companyLogo,
                },
            },
        }));
    };
    ...
    const unsaveJob = async (jobId: number) => {
        try {
            ...
            setSavedJobs(prev => prev.filter(savedJob => savedJob.Job.job_id !== jobId));
        } catch (err: any) {
            console.error('Failed to unsave job:', err);
            setError(err?.response?.data?.message || 'Failed to remove job from saved list');
        }
    };
