...
export const submitApplication = async (applicationData: ApplicationData): Promise<ApplicationResponse> => {
  ...
  formData.append('expected_salary', applicationData.expected_salary.toString());
  ...
  try {
    const response: AxiosResponse = await apiCall.post('/applications', formData, {
      headers: {
...
const normalizeApplication = (item: any): ApplicationResponse => {
  const expected_salary = typeof item?.expected_salary === 'string'
    ? parseInt(item.expected_salary)
    : (item?.expected_salary ?? 0);
  return {
    application_id: item?.application_id ?? item?.id ?? item?.applicationId ?? 0,
    expected_salary,
...
const normalizeListResponse = (data: any, page: number, limit: number): ApplicationListResponse => {
...
export const getUserApplications = async (
  ...
  try {
    ...
    const response: AxiosResponse = await apiCall.get(`/applications/my-applications?limit=${limit}&offset=${offset}`);
    ...
    if (status === 404 || status === 400 || status === 405) {
      try {
        const offset = (page - 1) * limit;
        const resp: AxiosResponse = await apiCall.get(`/applications/my-applications?limit=${limit}&offset=${offset}`);
        return normalizeListResponse(resp.data, page, limit);
      } catch (err2: any) {
        throw new Error(err2.response?.data?.message || 'Failed to fetch applications');
      }
    }
...
export const getJobApplicants = async (jobSlug: string): Promise<ApplicationListResponse> => {
  try {
    const response: AxiosResponse = await apiCall.get(`/applications/company/list/${jobSlug}`);
    return response.data;
  } catch (error: any) {
...
export const getApplicationDetail = async (applicationId: number): Promise<ApplicationResponse> => {
  try {
    const response: AxiosResponse = await apiCall.get(`/applications/detail/${applicationId}`);
    return response.data;
  } catch (error: any) {
    throw new Error(error.response?.data?.message || 'Failed to fetch application detail');
  }
};
...
export const updateApplicationStatus = async (
  ...
  try {
    const response: AxiosResponse = await apiCall.patch(`/applications/update/${status}/${applicationId}`);
    return response.data;
  } catch (error: any) {
...
export const checkApplicationExists = async (jobId: number): Promise<boolean> => {
  try {
    const response: AxiosResponse = await apiCall.get(`/applications/check/${jobId}`);
...
export const downloadCV = async (applicationId: number): Promise<Blob> => {
  try {
    const response: AxiosResponse = await apiCall.get(`/applications/${applicationId}/cv`, {
      responseType: 'blob'
    });
    return response.data;
  } catch (error: any) {
