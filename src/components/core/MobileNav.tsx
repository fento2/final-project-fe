...
const MobileNav = ({ setOpen, setActive, active }: IMobileNav) => {
  ...
  const { isLogin, setLogOut, email, role } = useAuthStore()
  const menus = [
    { label: "Home", href: "/" },
  ...
  return (
    <>
      {/* Overlay */}
      <div
        className="fixed inset-0 backdrop-blur-sm z-20 bg-black/40"
        onClick={() => setOpen(false)}
      />

      {/* Mobile Menu */}
      <div className="fixed top-0 right-0 h-full w-72 bg-white z-30 shadow-lg flex flex-col pt-6 px-6">
        {/* Logo */}
